...
if __name__ == '__main__':
    ...
    class MyDelegate(SymmetricTopologyDelegate):
        ...
        def level_of_details_neuron_nodes(self) -> List[NodePath]:
            ...
            files = lod_3D_mesh_object_filenames("assets_neural_network", TopologyObject.NEURON)
            return [self.base.loader.loadModel(filename) for filename in files]
            # return [self.base.loader.loadModel(f"assets_neural_network/neuron_1280T.obj")]

        def level_of_details_edge_nodes(self) -> List[NodePath]:
            # return a list of 3D mesh objects that defines
            # an edge and different levels of detail
            # From greatest to least level of detail
            files = lod_3D_mesh_object_filenames("assets_neural_network", TopologyObject.EDGE)
            return [self.base.loader.loadModel(filename) for filename in files]
            # return [self.base.loader.loadModel(f"assets_neural_network/cylinder_172T.obj")]
    ...
    class MyNeuralNetworkApp(ShowBase):

        def __init__(self, lighting_and_animation: bool):
            super(MyNeuralNetworkApp, self).__init__()
    ...
    app = MyNeuralNetworkApp(lighting_and_animation=True)
    app.run()
